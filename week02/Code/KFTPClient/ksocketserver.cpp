...
bool KSocketServer::bindPort(u_short port)
{
	...
	{
		std::cerr << "bind port failed, Error Code : " << WSAGetLastError() << std::endl;
		return false;
	}
	return true;
}
...
bool KSocketServer::listenSocket()
{
	...
	if (m_ret == SOCKET_ERROR)
	{
		std::cerr << "listen socket failed, Error Code : " << WSAGetLastError() << std::endl;
		return false;
	}
	return true;
}
...
KSocket* KSocketServer::acceptSocket()
{
	...
	if (m_csocket == -1)
	{
		std::cerr << "accept socket failed, Error Code : " << WSAGetLastError() << std::endl;
		return nullptr;
	}
